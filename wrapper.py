...
def readStat(path):
    statData = np.genfromtxt(path,skip_header=1, delimiter='\t',dtype='S5,i8,S7,i8,f8,i8,i8')
    ...
    for region in uniqueREGIONS:
        ...
        for coveragePoint in uniqueCOVERAG:
            ...
            if not len(newAr) == 0:
                percentileMin.append(newAr.min())
                percentile25.append(np.percentile(newAr, 25))
                percentile50.append(np.percentile(newAr, 50))
                percentile75.append(np.percentile(newAr, 75))
                percentileMax.append(newAr.max())
    ...
    plt.plot(np.array(xs), np.array(percentile50),'k',color='#CC4F1B')
    plt.fill_between(np.array(xs), np.array(percentile25), np.array(percentile75),
        alpha=0.5, edgecolor='#CC4F1B', facecolor='#FF9848')
    plt.fill_between(np.array(xs), np.array(percentileMin), np.array(percentileMax),
        alpha=0.5, edgecolor='#CC4F1B', facecolor='#FF9848')
...
def randStat():
    ...
    sampleStat.write("@GENE BASE_COV TISSUE SAMPLE FACTOR COV TMP\n")
        ...
        for factor in xfrange(0.1,1.0,0.1):
            ...
            for tissue in range(10):
                for sample in range(10):
                    TPM = random.randint(int(base_TPM*factor),int(base_TPM/factor))
                    LN = HD+"\tTISSUE"+str(tissue)+"\t"+str(sample)+"\t"+str(factor)+"\t"+str(coverage)+"\t"+str(TPM)+"\n"
                    sampleStat.write(LN)
...
# @GENE BASE_COV TISSUE SAMPLE FACTOR COV TMP
# GENE0  600 TISSUE0 0   0.1 60  6781
# GENE0  600 TISSUE0 1   0.1 60  6407
# GENE0  600 TISSUE0 2   0.1 60  15825
# GENE0  600 TISSUE0 3   0.1 60  16297
